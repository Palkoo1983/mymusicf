...
/* === VPOS Silent Flow === */
const pendingOrders = new Map(); // ref -> order payload (short-lived)
...
// Start VPOS: accept order payload, stash it, and 302 redirect to provider (or simulate)
app.post('/api/vpos/start', express.urlencoded({ extended: true }), (req, res) => {
  try {
    const order = Object.assign({}, req.body || {});
    // Minimal sanity keep
    const ref = makeRef(20);
    const createdAt = Date.now();
    pendingOrders.set(ref, { order, createdAt });

    // Build return URLs
    const host = (req.headers['x-forwarded-proto'] ? req.headers['x-forwarded-proto'] + '://' : 'http://') + (req.headers['x-forwarded-host'] || req.headers.host);
    const successUrl = host.replace(/\/+$/,'') + '/success.html?status=success&ref=' + encodeURIComponent(ref);
    const failUrl = host.replace(/\/+$/,'') + '/cancel.html?status=fail&ref=' + encodeURIComponent(ref);

    // If real provider base URL is configured, redirect there with ref and return URLs; otherwise simulate success page.
    const provider = process.env.VPOS_PROVIDER_URL || '';
    if (provider) {
      const redir = provider + '?ref=' + encodeURIComponent(ref) + '&success_url=' + encodeURIComponent(successUrl) + '&fail_url=' + encodeURIComponent(failUrl);
      res.redirect(302, redir);
    } else {
      // Simulation mode: go straight to success page (manual testing)
      res.redirect(302, successUrl);
    }
  } catch (e) {
    console.error('[VPOS START ERROR]', e);
    res.status(500).send(''); // No content to frontend
  }
});

// Provider callback webhook (server-to-server). Trigger background generation if success.
app.post('/api/vpos/callback', express.json(), async (req, res) => {
  try {
    const body = req.body || {};
    const ref = body.ref || body.reference || body.order_ref || (req.query && (req.query.ref || req.query.reference));
    const status = (body.status || '').toString().toLowerCase() || (req.query && (req.query.status || '')).toString().toLowerCase();
    if (!ref) return res.status(400).end();
    const rec = pendingOrders.get(ref);
    if (!rec) return res.status(204).end(); // unknown/expired is fine

    // expire old refs (30 mins TTL)
    if (Date.now() - rec.createdAt > 30*60*1000) { pendingOrders.delete(ref); return res.status(204).end(); }
    ...
    return res.status(204).end(); // Silent to client
  } catch(e){
    console.error('[VPOS CALLBACK ERROR]', e);
